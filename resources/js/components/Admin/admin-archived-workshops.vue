<template>
    <div class="pa-3">
        <v-select
            ...
            item-value="id"
            :label="$t('Teacher')"
            @update:modelValue="handleTeacherSelect"
            :loading="isLoading"
        />
        <v-text-field variant="outlined" :label="$t('Title')" v-model="title" :disabled="teacher==null"/>
        <div>
            <archived-workshop-preview 
            class="my-2" 
            v-for="workshop in archivedWorkshops"
            :workshop="workshop"
            @see="seeWorkshop"
...
<script setup>
    ...
    const { getTeacherArchive, duplicateWorkshop, deleteWorkshop } = workshopStore;
    const { archivedWorkshops, isLoading } = storeToRefs(workshopStore);
    ...
    const handleTeacherSelect = () => {
        getTeacherArchive(teacher.value);
    }
